...
2. **Publish `@polystate/definition`, `@polystate/cli`, `@polystate/generator-react`, `@polystate/generator-angular` at `1.0.0`.** This is the actual outstanding release blocker — the package most users touch first (`cli`) is still shipping pre-fix behavior. **Not done — requires npm publish credentials/authorization, holding for explicit go-ahead.**
3. Run `npm run changeset` (or equivalent) to backfill `1.0.0` changelog entries so they match what's actually on npm.
