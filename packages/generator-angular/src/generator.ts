...
export const initialState: ${storeName}State = ${JSON.stringify(definition.initialState, null, 2)
    .split('\n')
    .join('\n')};
...
function renameParam(src: string, oldName: string, newName: string): string {
  ...
  result = result.replace(new RegExp(`(?<![.\\w])\\b${oldName}\\b(?![\\w:])`, 'g'), newName);
  return result;
}
...
`;
  ...
  const effects = asyncActions.map((a) => {
    const switchMapParam =
      a.payloadParamName && a.payloadType ? `({ payload: ${a.payloadParamName} })` : '()';
